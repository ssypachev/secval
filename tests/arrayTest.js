...
describe("Should test array validator", () => {
	...
	it ("Should test valid array, min, max", () => {
		...
		let [err, options] = new Validator()
		.with(arg)
		.arg("a").required.array.min(1).build();
		...
		.with(arg)
		.arg("a").required.array.max(3).build();
		...
		[err, options] = new Validator()
		.with(arg)
		.arg("a").required.array.between(1,3).build();
	...
	it ("should test exact number of items in array", () => {
		...
		let [err, options] = new Validator()
		.with(arg)
		.arg("a").required.array.exactly(8).build();
	...
	it ("Should test invalid array, min, max", () => {
		...
		let [err, options] = new Validator()
		.with(arg)
		.arg("a").required.array.min(5).build();
		...
		[err, options] = new Validator()
		.with(arg)
		.arg("a").required.array.max(1).build();
	...
	it ("should test valid string array with strict mode", () => {
		...
		let [err, options] = new Validator()
		.with(arg)
		.arg("a").required.strict.array.build();
